...
def get_model(weights='imagenet'):
    ...
    fovea_input = Input(shape=(50, 50, 3), name='fovea_input')
    x = Conv2D(16, 3, 3, padding='same', activation='relu')(fovea_input)
    ...
    context_input = Input(shape=(14, 14, 3), name='context_input')
    c = Conv2D(32, 3, 3, padding='same', activation='relu')(context_input)
    c = Lambda(tf.nn.local_response_normalization,
               arguments={'alpha': 1e-4, 'bias': 2})(c)
    c = MaxPooling2D((2, 2), padding='same')(c)
    c = Conv2D(16, 3, 3, activation='relu')(c)
    c = Lambda(tf.nn.local_response_normalization,
               arguments={'alpha': 1e-4, 'bias': 2})(c)
    c = MaxPooling2D((2, 2), padding='same')(c)
    ...
    predictions = Dense(len(data.classes), activation='softmax')(cn)
